...
SQLITE_DB_PATH = os.path.join("data", "build_history.db") # Path to SQLite database file
...
def get_recent_build_history(num_builds=5):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT * FROM build_records ORDER BY timestamp DESC LIMIT ?",
            (num_builds,)
        )
...
def calculate_temporal_features(current_features, build_history):
    ...
    if build_history:
        ...
        valid_loc_changes = [
            rec['loc_changed'] for rec in build_history if rec.get('loc_changed') is not None
        ]
        if valid_loc_changes:
            temporal_features['avg_loc_changed_last_N'] = sum(valid_loc_changes) / len(valid_loc_changes)
        else:
        ...
        if len(build_history) >= 1 and build_history[0].get('build_duration_estimation_minutes') is not None:
        ...
        valid_test_pass_rates = [
            rec['test_pass_rate'] for rec in build_history if rec.get('test_pass_rate') is not None
        ]
        if valid_test_pass_rates:
            temporal_features['avg_test_pass_rate_last_N'] = sum(valid_test_pass_rates) / len(valid_test_pass_rates)
        else:
            temporal_features['avg_test_pass_rate_last_N'] = 1.0 # Default to perfect if no data
...
def extract_real_build_features():
    ...
    features.update(temporal_feats) # Merge temporal features into the main features dictionary
